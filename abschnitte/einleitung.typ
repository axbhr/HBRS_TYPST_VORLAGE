// ─── Einleitung ──────────────────────────────────────────────────────────────
// Entspricht abschnitte/Einleitung.tex
#import "abkuerzungsverzeichnis.typ": acf, acs, acl, acp
...
Blies takte uhr bibel winde all stuck wette nie wie. Licht se dahin indem seine ku zu karte. Kurios bin minder kam diesen stille den schien. Lief laut so du meer da. Ku er wu offnung gesicht wachsam du zuhorer. Geschwatzt am arbeitsame ei vormittags hufschmied mi jahreszeit auskleiden. Leber ein funfe dem gutes. Brauchen bummelte in kurioses gepflegt launigen zu. Ja so aufgespart um ja fluchtigen betrachtet lattenzaun. @BarAhl08
...
#figure(
  table(
    ...
    align: center,
    // Kopfzeile
    [P], [Q], $not$, [(], [P], $or$,  [Q], [)], $or$,  [(], $not$, [P], $and$, [Q],
    // Datenzeilen
    [0], [0], [1], [0], [0], [0], [$bold(1)$], [1], [0], [0], [0], [], [$bold(1)$], [0],
    [0], [1], [0], [0], [1], [1], [$bold(1)$], [1], [0], [1], [1], [], [$bold(1)$], [0],
    [1], [0], [0], [1], [1], [0], [$bold(0)$], [0], [1], [0], [0], [], [$bold(0)$], [1],
    [1], [1], [0], [1], [1], [1], [$bold(0)$], [0], [1], [0], [1], [], [$bold(0)$], [1],
  ),
  caption: [Wahrheitstabelle: $not (P or Q) or (not P and Q) equiv not P$],
) <fig-wahrheitstabelle>
...
Hier steht ein Text. @BarAhl08 @BruMurPerWygMcN09
